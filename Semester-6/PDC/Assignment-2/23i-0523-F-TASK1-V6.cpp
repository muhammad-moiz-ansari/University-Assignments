...
#pragma omp parallel for schedule(dynamic, 16) reduction(+ : total_count)
  for (int vi = 0; vi < V; ++vi) {
    ...
    for (int j = 0; j < sizeA; ++j) {
      ...
      // Intersect count (OMP SIMD version)
...
// ================ RUNNER FUNCTION ================
void runOMP_ompSIMDVersion(const vector<int> &offset, vector<int> &neighbors,
                           int V) {
  cout << "Running OpenMP + omp SIMD version...\n";
...
int main() {
  ...
  runOMP_ompSIMDVersion(offset, neighbors, V);
